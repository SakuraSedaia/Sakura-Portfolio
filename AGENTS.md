...
- `src/components/sections/common/`: Shared layout sections such as nav and footer.
- `src/components/sections/index/`: Home page sections.
- `src/components/styles/`: SCSS partials imported by `src/app.scss`.
- `src/components/styles/common/`: Shared layout and component partials.
- `src/components/styles/pages/`: Page-specific partials.
...
- Use the project `Link` component from `~/components/links` for app links unless a lower-level router primitive is needed.
...
- Add page metadata with `Metadata` from `~/components/page-metadata` on user-facing routes.
- Prefer directory-based parameterized routes when adding dynamic pages, for example `src/routes/changelog/[changelog].tsx`.
...
- Import SCSS partials from `src/app.scss`.
- Keep SCSS partials under `src/components/styles/` unless a new local convention is introduced intentionally.
- Use page partials in `src/components/styles/pages/` and shared/common partials in `src/components/styles/common/`.
...
- Prefer existing CSS custom properties defined in `:root` in `src/app.scss`, for example `var(--accent)` or `var(--tx-1)`.
- Keep new global design tokens in `:root` in `src/app.scss` until a dedicated variables partial exists.
...
- Use the `~/*` alias for source imports, for example `~/components/links` and `~/utils/url-prefixer`.
- Use relative imports for local stylesheet partials from `src/app.scss`, matching the existing `@use "components/styles/..."` pattern.
...
- Use `pnpm dev` to run the local dev server on port `3333`.
  - `3232` (The port used in the `package.json`) is for the user's local dev server that's almost always running.
