...
Version 4 Alpha 3 is largely focused on stability improvements and cleanup, ensuring that all features from V3 are
implemented carefully and properly.

### Added

- Added new "Online Access Disabled" warning panel, since SACR Utils includes features which require internet access,
  and without permission the features cannot function properly.
...
- Updated `update_collection` logic in `OP_rig_rename` to use a toggle in user preferences instead of a hardcoded value
  in the UI.
...
- Moved Profile Creation logic to a popup modal window, which allows the user to either download a new profile from
  Mojang or import a skin file into a new profile.
